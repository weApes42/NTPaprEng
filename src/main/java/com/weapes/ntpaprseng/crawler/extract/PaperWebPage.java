...
public class PaperWebPage extends WebPage {
    ...
    private String parsePageBegin(final Document dom) {
        ...
        return pageRange.contains("–") == true ? pageRange.substring(0, pageRange.indexOf("–")) : pageRange;
    }

    private String parsePageEnd(final Document dom) {
        final String pageRange = dom.select(PAGE_CSS_SELECTOR).text();
        return pageRange.contains("–") == true ? pageRange.substring(pageRange.indexOf("–") + 1) : pageRange;
    }
